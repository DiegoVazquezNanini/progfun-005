...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = chars match {
    case List() => List()
    case x::xs => count(x,chars)::times(remove(x,xs))
  }

  def remove(c: Char, chars: List[Char]):List[Char] = chars match {
    case List() => List()
    case x::xs => if (x == c) remove(c, xs) else x::remove(c, xs)
  }

  def count(c: Char, chars: List[Char]): (Char, Int) = chars match {
    case List() => (c, 0)
    case x::xs => if (x == c) (c,1 + count(c,xs)._2) else (c, count(c,xs)._2)
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
    case Nil => Nil
    case x::xs => isort(makeLeafList(freqs))
  }
  ...
  def until(singleton: List[CodeTree] => Boolean, combine:List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = trees match {
    case List() => List()
    case _  => if (singleton(trees)) trees else until(singleton, combine)(combine (trees))
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodePos(tree: CodeTree, position:CodeTree, bits: List[Bit]): List[Char] = {
      tree match {
        case Leaf(c, _) => c :: decodePos(tree, tree, bits)
        case Fork(l, r, _, _) => bits match {
          case List() => Nil
          case x :: xs => if (x == 0) decodePos(tree, l, xs) else decodePos(tree, r, xs)
        }
      }
    }
    decodePos(tree,tree,bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodePos(tree: CodeTree, position: CodeTree, text: List[Char]): List[Bit] = { position match {
        case Leaf(c, _) => text match {
          case List() => Nil
          case x::xs  => encodePos(tree, tree, xs)
        }
        case Fork(l, r, c, _) => text match {
          case List() => Nil
          case x :: xs => if (chars(l).contains(x)) 0 :: encodePos(tree, l, text) else 1 :: encodePos(tree, r, text)
        }
      }
    }
    encodePos(tree,tree,text)
  }
  ...
  //List(('a',List(0,0,1,1,1)),('b',List(1,0,1)))
  /**
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    a.map(entry => entry match {
      case (c, b) => (c, 0 :: b)
    }) ::: b.map(entry => entry match {
      case (c, b) => (c, 1 :: b)
    })
  }
